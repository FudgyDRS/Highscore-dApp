...
contract SirLeonidus_P2E is ERC20, Ownable {
    ...
    function getVersion() public view returns(string) { return version; }
    function setVersion(string _version) public payable onlyOwner { version = _version; }
    ...
    function bytes32ToUint256(bytes bs, uint start) internal pure returns (uint) {
        require(bs.length >= start + 32, "slicing out of range");
        uint x;
        assembly { x := mload(add(bs, add(0x20, start))) }
        return x;
        }
    function getNftWeight(address _owner) public payable returns(bytes32){
    ...
    function addPlayerHighscore(uint256 _score, address _player, uint256 _timestamp) internal returns (bool) {
        ...
        if(playerHighscore[address].score == 0) {
    ...
    function addScore(uint256 _score, address _player) public payable {
        // add score
        // add to player highscores
        // add to daily highscores
        // add to all-time highscores
        // test NFTs if applicable
        scores.push((_score, block.timestamp, getVersion()));
        scoreIndex[_player].push(scores.size);

        // test if should payout or reset
        ...
        if(newDay) { resetHighscores(); }

        // check highscore status
        if(addPlayerHighscore(_score, _player, _timestamp))
            if(addHighscoreDaily(_score, _player, _timestamp))
                addHighscore(_score, _player, _timestamp)
    ...
    function getHighscore(address _player) public view returns (uint256, uint256, string) { return ( highscore[_player].score, highscore[_player].timestamp, highscore[_player].version ); }
    function getHighscores() public view returns (uint256[] _scores, address[] _players, uint256[] _timestamp, string[] memory _version) {
        for(int i=0; i<30; i++) {
            _scores.push(highscore[i].score);
            _players.push(highscore[i].player);
            _timestamps.push(highscore[i].timestamp);
            _version.push(highscore[i].version);
        }
        return (_scores, _players, _timestampes, _version);
        }
    function getHighscoresDaily() public view returns (uint256[] _scores, address[] _players, uint256[] _timestamp, string[] memory _version) {
        for(int i=0; i<30; i++) {
            _scores.push(highscoreDaily[i].score);
            _players.push(highscoreDaily[i].player);
            _timestamps.push(highscoreDaily[i].timestamp);
            _version.push(highscoreDaily[i].version);
        }
        return (_scores, _players, _timestampes, _version);
        }
    function getScores(address _player) public view returns (uint256[], uint256[]) {
        ...
        for(int i=0; i<_scores.size; i++) {
            _scores.push(_indecies[i]);
            _timestamps.push(_indecies[i]);\
            .add(_indecies[i]);
        }
